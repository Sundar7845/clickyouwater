...
class OrdersController extends Controller
{
    ...
    public function placeOrder(Request $request)
    {
        ...
        try {
            ...
            if ($request->coupon_id && strtolower($request->coupon_code_type) == "coupon") {
                ...
                if (strtolower($request->payment_through) == 'wallet') {
                    $this->updateWallet(Auth::user()->id, $request->offer_points_used, WalletTransactionTypes::WalletPending);
            ...
            if ($status == StatusTypes::OrderPlaced) {
                ...
                $this->sendHubNotification($orders->id, $request->status_id, $request->hub_id);
            }
    ...
    public function getRazorpayOrderId(Request $request)
    {
        try {
            ...
            $this->Log(__FUNCTION__, request()->method(), $e->getMessage(), Auth::user()->id, request()->ip(), gethostname(), 1);
            //TODO: Error message response not implemented.
            return null;
        }
    }
        ...
        try {
            ...
            if ($cancel_order_count > 0) {
                ...
                if (strtolower($order->coupon_code_type) == 'referral') {
                    ...
                    // $this->updateWallet(Auth::user()->id, $detect_wallet_points, WalletTransactionTypes::RemoveReferral);
                    //Delete user referral history
                    UserReferralHistory::where('user_id', $order->user_id)->delete();
                }

                //Update company ledger closing balance
                // $this->updateClosingBalance($order->transaction_amount);
                // $this->addClosingBalance($order->wallet_points_used);
                $orderDet = OrderDet::where('order_id', $request->order_id)->get();
                foreach ($orderDet as $key => $orderdet) {
                    ...
                    HubStock::updateOrCreate([
                        ...
                        'hub_id' => $order->hub_id,
                        'product_id' => $orderdet->product_id,
                        'order_qty' => DB::raw('order_qty - ' . $orderdet->qty)
                    ]);
                }
                ...
                $this->sendHubNotification($request->order_id, StatusTypes::Cancelled, $order->hub_id);
            }
            $response = array(
        ...
        try {
            ...
            if (strtolower($request->transaction_status) == 'success') {
                ...
                if ($order->coupon_id && strtolower($order->coupon_code_type) == "coupon") {
                    ...
                    $this->addReferralHistory(Auth::user()->id, $referred_by);
                    $this->updateWallet(Auth::user()->id, $order->total_discount_amount, WalletTransactionTypes::WalletPending);
                }
                $orderDet = OrderDet::where('order_id', $request->order_id)->get();
                foreach ($orderDet as $key => $orderdet) {
                    //Store Order Data in HubStock
            ...
            if ($request->status_id == StatusTypes::OrderPlaced) {
                $this->sendHubNotification($request->order_id, $request->status_id, $order->hub_id);
            }
            $response = array(
                'message' => "Updated Successfully",
                'data' => [],
