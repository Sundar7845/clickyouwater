...
class ExpenseController extends Controller
{
  ...
  {
    try {
      $expenseVendor = Ledger::where('ledger_type_id', 2)->where('is_active', 1)->whereNull('deleted_at')->get();
      $expenseGroup = ExpenseGroup::get();
      $companyLedger = Ledger::where('ledger_type_id', 1)->where('is_active', 1)->whereNull('deleted_at')->get();
  ...
  public function expenseCreate(Request $request)
  {
    ...
    try {
      ...
      $last_insert_expense_id = Expense::insertGetId([
        ...
        'amount_paid' => ($request->isPaid == 1 ? $request->txtAmount : 0),
  ...
  {
    try {
      $expenseGroup = ExpenseGroup::where('is_active', 1)->whereNull('deleted_at')->get();
      $companyLedger = Ledger::where('ledger_type_id', 2)->where('is_active', 1)->whereNull('deleted_at')->get();
