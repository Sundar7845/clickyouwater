...
class PaymentsController extends Controller
{
  ...
  public function payments()
  {
    try {
      ...
      $bankName = Bank::whereNull('deleted_at')->get();
  ...
  public function paymentsForExpense(Request $request)
  {
    try {
      $generalExpenseData = Ledger::select('ledgers.*', 'expenses.amount_paid', 'expenses.amount')
        ...
        ->where('ledgers.is_active', 1)->whereNull('ledgers.deleted_at')
        ->distinct()
        ->get();
      $employeeExpenseData = User::select('users.*', 'expenses.amount_paid', 'expenses.amount')
        ->join('expenses', 'expenses.employee_user_id', 'users.id')
        ->where('expenses.is_paid', '!=', 1)
        ->where('expenses.is_cancelled', 0)
  ...
  {
    try {
      $vendorPendingBills = Expense::where('ledger_id', $request->vendor_id)
        ->where('is_paid', 0)
        ->where('is_cancelled', 0)
        ->whereColumn('amount', '>', 'amount_paid')->get();
      $employeePendingBills = Expense::where('employee_user_id', $request->employee_id)
        ->where('is_paid', 0)
        ->where('is_cancelled', 0)
        ->whereColumn('amount', '>', 'amount_paid')->get();
      return response()->json([
  ...
  public function createPayment(Request $request)
  {
    ...
    try {
      ...
      $lastInsertedPaymentId = Payment::insertGetId([
        ...
        'bank_id' => ($request->ddlBankName == 0 ? null : $request->ddlBankName),
      ...
      $paymentDetData = json_decode($request->hiddenPaymentDet, true);

      $expenseIds = array_column($paymentDetData, 'expenseId');

      $expenses = Expense::whereIn('id', $expenseIds)->get();

      foreach ($paymentDetData as $paymentDet) {
        foreach ($expenses as $exp) {
          if ($exp->id == $paymentDet['expenseId']) {
            $newAmountPaid = $exp->amount_paid + $paymentDet['amount'];
            Expense::findOrFail($paymentDet['expenseId'])->update([
              'amount_paid' => $newAmountPaid
            ]);
            PaymentDet::create([
              'payment_id' => $lastInsertedPaymentId,
              'expense_id' => $paymentDet['expenseId'],
  ...
  {
    try {
      $generalExpenseData = Ledger::where('ledger_type_id', 2)->where('is_active', 1)->whereNull('deleted_at')->get();
