...
class LogisticController extends Controller
{
   ...
   public function logisticPartner($id = NULL)
   {
      try {
         ...
         $hub = Hub::whereNotIn('id', $allocatedHubIds)->whereNull('deleted_at')->get();
         ...
         if ($id) {
            ...
            $hub_id = Hub::whereNotIn('id', $allocatedHubIds)->whereNull('deleted_at')->pluck('id')->toArray();
            $mergeHub = array_merge($hubconfiq, $hub_id);

            $hub = Hub::select('id', 'hub_name')
               ->whereIn('id', $mergeHub)
      ...
      try {
         ...
         return datatables()->of($logisticData)
            ->addColumn('action', function ($row) {
               ...
               if ($this->isUserHavePermission(MenuPermissionType::Delete) && $row->logistic_driver_count < 1 && $row->logistic_vehicle_count < 1) {
   ...
   public function activeStatus($id, $status)
   {
      ...
      try {
         ...
         if ($status == 0) {
            $this->inactiveUser($id, RoleTypes::LogisticPartner);
         } else {
            $this->activeUser($id, RoleTypes::LogisticPartner);
         }
         DB::commit();
      } catch (\Exception $e) {
         DB::rollBack();
