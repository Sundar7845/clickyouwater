...
class BannerController extends Controller
{
    ...
    public function addBanner(Request $request)
    {
        $request->validate([
            'txtBannerName' => [
                'required',
                Rule::unique('banners', 'banner_name')->ignore($request->hdBannerId, 'banner_name'),
            ],
            'txtStartDate' => 'required',
            'txtEndDate' => 'required|after:txtStartDate',
        ], [
            'txtBannerName.unique' => 'Banner name already exists.',
            'txtEndDate.after' => "The end date must be after start date."
        ]);
        ...
        try {
            ...
            if ($request->hdBannerId == null) {
                ...
                Banners::findorfail($request->hdBannerId)->update([
                    'Banner_name' => $request->txtBannerName,
                    'banner_img' => $request->hasFile('bannerImage') ? $this->fileUpload($file, 'upload/settings/banners', $fileName) : $oldImage,
                    'start_date' => $request->txtStartDate,
                    'end_date' => $request->txtEndDate,
