...
class ManufacturerController extends Controller
{
    ...
    public function manufacturer($id = NULL)
    {
        try {
            ...
            if ($id) {
                ...
                $cities = $this->getCities($man->state_id, 0);
                //Get Areas against city
                $areas = $this->getAreas($man->city_id, 0);
                $manExample = $man->manufacturer_code;
                return view('admin.manufacturer_management.manufacture_edit', compact('bindDocuments', 'man', 'manExample', 'states', 'cities', 'areas'));
            }
            $man =  null;
    ...
    public function manufactureCreate(Request $request)
    {
        ...
        try {
            ...
            if ($request->chkThirdpartyTieup == 1) {
                $totalturnover = $request->txtTotalTurnover;
            } else {
                $totalturnover = 0;
            }
            ...
            $this->createUser(
                ...
                $request->txtMobile,
                RoleTypes::Manufacturer,
                1,
                Auth::user()->id
            );

            //Create QR Code for Manufacturer
            $this->generateQrCode($last_inserted_manufacturer_id, $request->txtManufacturerId, DocumentModulesType::Manufacturer);
    ...
    private function createManufactureDocuments(Request $request, $documents, $manufacture_id)
    {
        try {
            foreach ($documents as $doc) {
                ...
                $documentPath = null;
                if ($request->hasFile('file_' . $doc->id)) {
                    $documentPath = $this->fileUpload($file, 'upload/Manufacture/' . $request->txtManufacturerId, $fileName);
    ...
    public function manufactureUpdate(Request $request)
    {
        ...
        try {
            ...
            //Create QR Code for Manufacturer
            $this->generateQrCode($request->id, $request->txtManufacturerId, DocumentModulesType::Manufacturer);

            //Update Manufacturer Login Account For User
            $this->updateUser(
                $request->txtMobile,
                $request->txtOffEmail,
                $request->txtMobile,
    ...
    public function manufacturerList(Request $request)
    {
        try {
            //Get states
            $states = $this->getStates();
            $manufacturer = Manufacturer::whereNull('deleted_at')->get();
            //To load list based type(All,Today,ThisMonth)
            $type = $request->input('type');
            return view('admin.manufacturer_management.manufacturerlist', compact('type', 'states', 'manufacturer'));
        } catch (\Exception $e) {
            $this->Log(__FUNCTION__, "GET", $e->getMessage(), Auth::user()->id, request()->ip(), gethostname());
        }
    }
    public function manufacturerData(Request $request)
    {
        try {
            ...
            $query = DB::table('manufacturers as m')
                ...
                ->leftjoin('manufacturer_documents as md', 'md.manufacture_id', 'm.id')
                ->select(
                    'm.*',
                    's.state_name',
                    'c.city_name',
            ...
            if ($request->type === 'today') {
                $query = $query->whereDate('m.created_at', today());
            } elseif ($request->type === 'thismonth') {
                $query = $query->whereMonth('m.created_at', now()->month);
            }
            ...
            foreach ($manufacturerData as $manufacturer) {
                ...
                $hubCount = DB::table('hub_manufacture_configs')
                    ->where('manufacturer_id', $manufacturerId)
                    ->count();
                $logisticPartnerCount = DB::table('logistics_manufacture_configs')
                    ->where('manufacture_id', $manufacturerId)
                    ->count();
                $manufacturer->hub_count = $hubCount;
                $manufacturer->logistic_partner_count = $logisticPartnerCount;
            }
            ...
            return datatables()->of($manufacturerData)
                ->addColumn('action', function ($row) {
                    ...
                    if ($this->isUserHavePermission(MenuPermissionType::Delete) && $row->hub_count < 1 && $row->logistic_partner_count < 1) {
    ...
    public function activeStatus($id, $status)
    {
        ...
        try {
            ...
            if ($status == 0) {
                $this->inactiveUser($id, RoleTypes::Manufacturer);
            } else {
                $this->activeUser($id, RoleTypes::Manufacturer);
            }
            DB::commit();
        } catch (\Exception $e) {
            DB::rollback();
    ...
    public function deleteManufacturer($id)
    {
        ...
        try {
            $references = [
                LogisticBooking::class, LogisticsManufactureConfig::class, LogisticTrip::class,
                ManufactureStock::class, ManufactureStockHistory::class, StockInProduction::class, StockOutward::class
            ];

            $count = 0;
            $hubManufactureCount = HubManufactureConfig::where('manufacturer_id', $id)->get()->count();
            foreach ($references as $reference) {
                $count += $reference::where('manufacture_id', $id)->get()->count();
            }
            if ($count === 0 && $hubManufactureCount === 0) {
