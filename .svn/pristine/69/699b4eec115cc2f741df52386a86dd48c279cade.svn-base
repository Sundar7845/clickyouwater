...
class DeliveryPersonController extends Controller
{
    ...
    public function activeStatus($id, $status)
    {
        try {
            ...
            if ($status == 0) {
                $this->inactiveUser($id, RoleTypes::DeliveryPerson);
            } else {
                $this->activeUser($id, RoleTypes::DeliveryPerson);
            }
        } catch (\Exception $e) {
    ...
    public function Deletedeliveryperson($id)
    {
        ...
        try {
            ...
            $this->Log(__FUNCTION__, "GET", $e->getMessage(), Auth::user()->id, request()->ip(), gethostname());
            $notification = array(
                'message' => 'Delivery Person Could Not Be Deleted!',
                'alert' => 'error'
            );
            return response()->json([
                'responseData' => $notification
    ...
    {
        $allocated_vehicles = DeliveryVehicleInfo::pluck('hub_vehicle_info_id')->toArray();
        $hubVehicleInfo = HubVehicleInfo::where('hub_id', $request->hub_id)
            ->whereNotIn('id', $allocated_vehicles)->get();
        return response()->json([
