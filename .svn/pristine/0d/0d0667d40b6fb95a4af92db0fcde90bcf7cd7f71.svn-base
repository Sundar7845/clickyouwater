...
class LedgerController extends Controller
{
  ...
  {
    try {
      $ledgerData = Ledger::select('ledgers.*', 'ledger_types.ledger_type', 'expenses.ledger_id as exp_ledger_id', 'payments.ledger_id as pay_ledger_id')
        ->join('ledger_types', 'ledger_types.id', 'ledgers.ledger_type_id')
        ->leftJoin('expenses', 'expenses.ledger_id', 'ledgers.id')
        ->leftJoin('payments', 'payments.ledger_id', 'ledgers.id')
      ...
      return datatables()->of($ledgerData)
        ->addColumn('action', function ($row) {
          ...
          if ($this->isUserHavePermission(MenuPermissionType::Delete) && $row->ledger_type_id != 1 && $row->exp_ledger_id == null && $row->pay_ledger_id == null) {
  ...
  public function deleteLedger($id)
  {
    DB::beginTransaction();
    try {
      $expenseLedgerCount = Expense::where('ledger_id', $id)->orWhere('company_ledger_id', $id)->get()->count();
      $paymentLedgerCount = Payment::where('ledger_id', $id)->orWhere('company_ledger_id', $id)->get()->count();

      if ($expenseLedgerCount == 0 && $paymentLedgerCount == 0) {
