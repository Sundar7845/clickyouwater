...
class DeliveryPersonController extends Controller
{
    ...
    public function confirmDelivery(Request $request)
    {
        ...
        try {
            ...
            if ($order->orderDets) {
                foreach ($order->orderDets as $key => $orderDet) {
                    if ($orderDet->products->is_emptycan_return) {
                        ...
                        $jsonString = json_encode($request->damaged_cans);
            ...
            $this->addClosingBalance($order->wallet_points_used + $order->transaction_amount);
            ...
            if (strtolower($order->coupon_code_type) == 'referral') {
                $referred_by = User::where('referral_code', $order->coupon_code)->value('id');
