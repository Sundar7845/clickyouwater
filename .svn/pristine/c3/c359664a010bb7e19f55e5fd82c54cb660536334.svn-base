...
(function () {
  ...
  if (typeof earningReportsTabsIncomeEl !== 'undefined' && earningReportsTabsIncomeEl !== null) {
    ...
    fetch('yearlysalesdata')
      ...
      .then(data => {
        // Update the chart series data and x-axis categories with the fetched data
        earningReportsTabsIncomeConfig.series = [
          {
            name: 'Expense Amount',
            data: data.map(item => item.expense_amount),
          },
        ];
        earningReportsTabsIncomeConfig.xaxis.categories = data.map(item => item.month);

        // Create and render the chart
        const earningReportsTabsSales = new ApexCharts(earningReportsTabsIncomeEl, earningReportsTabsIncomeConfig);
        earningReportsTabsSales.render();
      });
  }
