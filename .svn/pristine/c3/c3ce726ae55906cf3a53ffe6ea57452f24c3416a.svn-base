...
class WalletTransactionThroughController extends Controller
{
    ...
    public function walletTransactionThrough()
    {
        try {
            ...
            return view('admin.settings.wallet_transaction_through', compact('walletTransactionType'));
        } catch (\Exception $e) {
    ...
    public function getWalletTransactionThroughById($id)
    {
        try {
            $walletTransactionThrough = WalletTransactionThrough::select('wallet_transaction_throughs.*', 'wallet_transaction_types.wallet_transaction_type')
                ->join('wallet_transaction_types', 'wallet_transaction_types.id', '=', 'wallet_transaction_throughs.wallet_transaction_type_id')
                ->where('wallet_transaction_throughs.id', $id)
                ->first();

            if ($walletTransactionThrough) {
                return response()->json([
                    'walletTransactionThrough' => $walletTransactionThrough
                ]);
            } else {
                return response()->json([
                    'message' => 'Wallet transaction through not found'
                ], 404);
            }
        } catch (\Exception $e) {
            $this->Log(__FUNCTION__, 'GET', $e->getMessage(), Auth::user()->id, request()->ip(), gethostname());
