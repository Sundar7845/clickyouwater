...
class DocumentController extends Controller
{
    ...
    public function createDocuments(Request $request)
    {
        ...
        try {
            ...
            $authLogisticPartnerRedirectUrl = Auth::user()->role_id == RoleTypes::LogisticPartner ? 'documents' : 'logisticdocuments';
            DB::commit();
            return redirect()->route($authLogisticPartnerRedirectUrl)->with($notification);
        } catch (\Exception $e) {
            DB::rollBack();
