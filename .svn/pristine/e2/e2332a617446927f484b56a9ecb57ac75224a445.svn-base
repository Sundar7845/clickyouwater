...
function LoadMenus(id) {
    ...
    jQuery.ajax({
        ...
        $.each(menuData.menu, function (idx, val) {
            ...
            List += "<td>" + ((val.group_name == null || val.group_name == "") ? "" : val.group_name + ' - ') + val.menu_name + "<input type='hidden' name='menuId[]' value=" + val.id + "></td>";
...
// jquery Validation
$(function () {
    $("form[name='permissionList']").validate({
        rules: {
