...
class AssetController extends Controller
{
    ...
    public function asset()
    {
        try {
            ...
            $assetData = Asset::select('assets.*', 'asset_types.prefix')
                ->join('asset_types', 'asset_types.id', 'assets.asset_id')
    ...
    public function addAsset(Request $request)
    {
        $request->validate([
            ...
            'txtAssetName' => [
                'required',
                Rule::unique('assets', 'asset_name')->ignore($request->hdAssetId)->whereNull('deleted_at'),
            ],
    ...
    {
        try {
            $assetData = Asset::select('assets.*', 'asset_types.asset_type', 'employee_assets.employee_id')
                ->join('asset_types', 'asset_types.id', 'assets.asset_type_id')
                ->leftJoin('employee_assets', 'employee_assets.asset_id', 'assets.id')
                ->whereNull('assets.deleted_at')
                ->groupBy('assets.id')
            ...
            return datatables()->of($assetData)
                ...
                onclick="doEdit(' . $row->id . ');"></i> ';
                    ...
                    if ($this->isUserHavePermission(MenuPermissionType::Delete) && $row->employee_id == null) {
