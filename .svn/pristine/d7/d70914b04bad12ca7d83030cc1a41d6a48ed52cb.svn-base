...
class AuthController extends Controller
{
    ...
    public function login(Request $request)
    {
        ...
        try {
            // Check email
            $user = User::where('mobile', $fields['mobile'])->where('is_active', 1)->first();
    ...
    public function verifyOTP(Request $request)
    {
        ...
        try {
            ...
            // Check email
            $user = User::where('mobile', auth()->user()->mobile)->where('is_active', 1)->first();
    ...
    public function verifyMobile(Request $request)
    {
        ...
        try {
            $user = User::where('mobile', $mobile)
                ...
                // ->when($role_id == RoleTypes::Customer, function ($query) {
                //     return $query->where('role_id', RoleTypes::Customer);
                // })
                // ->when($role_id == RoleTypes::DeliveryPerson, function ($query) {
                //     return $query->where('role_id', RoleTypes::DeliveryPerson);
                // })
                // ->when($role_id == RoleTypes::Manufacturer, function ($query) {
                //     return $query->where('role_id', RoleTypes::Manufacturer);
                // })
                // ->when($role_id == RoleTypes::Hub, function ($query) {
                //     return $query->where('role_id', RoleTypes::Hub);
                // })
                // ->when($role_id == RoleTypes::LogisticPartner, function ($query) {
                //     return $query->whereIn('role_id', [RoleTypes::LogisticPartner, RoleTypes::Driver]);
                // })
                ->first();
            // dd($user);
            if ($user) {
