...
var city_id =
 ...
 function binddata(){
    var brands="";
    $.each(jlist, function (arrIndex, arrValue) {
 ...
 function doSearch() {
    jlist = temp;
    jlist = $.grep(jlist, function (val,idx) {
        return (val.category_name.indexOf($.trim($("#txtSearch").val())) != -1 || val.product_type_name.toLowerCase().indexOf($.trim($("#txtSearch").val())) != -1|| val.brand_name.indexOf($.trim($("#txtSearch").val())) != -1);
    });
            ...
            $.each(result, function (key, value) {
                $.each(value, function (arrIndex, arrValue) {
                    $.each(arrValue, function (darrIndex, darrValue) {
                        allocatedbrand += "<tr id='trbrandallocated"+darrValue.id+"' BID="+darrValue.brand_id+" CNM='" + darrValue.category_name + "' PNM='" + darrValue.product_type_name + "' BNM='" + darrValue.brand_name + "'>";
                        allocatedbrand += "<td><input type='hidden' class='brandallocated' id='taballocatedbrandname' name='taballocatedbrandname[]' value='"+darrValue.id+"'>"+darrValue.brand_name+"</td>";
                        allocatedbrand += "<td><a><i style='cursor:pointer'; class='text-danger ti ti-trash me-1' onclick='doRemove("+darrValue.id+");'></i></a></td>";
                        allocatedbrand += "</tr>";
                        RowIndex++;
                    });
                });
